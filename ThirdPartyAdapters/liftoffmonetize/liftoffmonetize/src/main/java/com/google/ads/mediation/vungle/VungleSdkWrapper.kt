...
object VungleSdkWrapper {
  ...
  fun logCustomSizeForBannerPlacement(
    ...
    if (
      !VungleAds.isInline(placementId) &&
        !(adSize.width == AdSize.BANNER.width && adSize.height == AdSize.BANNER.height) &&
        !(adSize.width == AdSize.MEDIUM_RECTANGLE.width &&
          adSize.height == AdSize.MEDIUM_RECTANGLE.height) &&
      ...
      val customSizeMismatchMessage =
        "CustomBannerSizeMismatch:w-${adSize.width}|h-${adSize.height}"
      VungleMediationLogger.logError(bannerAdView, customSizeMismatchMessage)
      Log.e(
        VungleMediationAdapter.TAG,
        "Please use a Liftoff inline placement ID in order to use custom size banner: placementId=$placementId adSize=$adSize",
      )
    }
  }
}
